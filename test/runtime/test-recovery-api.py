...
def main(env_file=None):
    ...
    with tempfile.TemporaryDirectory(prefix=RUN_ID) as temporary:
        ...
        try:
            docker("pull", GLUETUN)
            network = docker("network", "create", "--label", f"{LABEL}={RUN_ID}", RUN_ID).stdout.strip()
            ...
            if env_file is None:
                ...
                resolved = docker("compose", "--env-file", str(env_file), "--file",
                                  str(ROOT / "docker-compose.yml"), "config", "--environment").stdout
                values = dict(line.split("=", 1) for line in resolved.splitlines() if "=" in line)
                if values.get("PIA_USER", "") in ("", "p1234567") or values.get("PIA_PASS", "") in ("", "abc123", "shiverMeTimbers123"):
            ...
            if env_file is not None:
                privateerr = docker(
                    "create", "--name", f"{RUN_ID}-privateerr", "--label", f"{LABEL}={RUN_ID}",
                    "--network", RUN_ID, "--volume", f"{config}:/gluetun",
                    "--env", "PIA_USER", "--env", "PIA_PASS", "--env", "PRIVATEERR_GLUETUN_API_KEY",
                    "--env", "PRIVATEERR_AUTO_RECOVER=true", "--env", "PRIVATEERR_KEEPALIVE=true",
                    "--env", "VPN_PROTOCOL=wireguard", "--env", "PIA_CONNECT=false",
                    "--env", "PIA_PF=true", "--env", "PIA_DNS=true", "--env", "DIP_TOKEN=no",
                    "--env", "DISABLE_IPV6=no", "--env", "AUTOCONNECT=false",
                    "--env", "PRIVATEERR_RECOVERY_INTERVAL=2", "--env", "PRIVATEERR_RECOVERY_FAILURE_SECONDS=15",
                    "--env", "PRIVATEERR_RECOVERY_COOLDOWN=30", IMAGE,
                ).stdout.strip()
                containers.append(privateerr)
                docker("start", privateerr)
                for _ in range(100):
                    if docker("exec", privateerr, "privateerr-healthcheck", check=False).returncode == 0:
                        break
                    time.sleep(2)
                else:
                    raise RuntimeError("Privateerr did not generate an initial PIA configuration")
                print("PASS: Privateerr generated a real PIA registration using the default ca region.", flush=True)
            forwarding = "on" if env_file else "off"
            gluetun = docker(
                "create", "--name", f"{RUN_ID}-gluetun", "--label", f"{LABEL}={RUN_ID}",
                "--network", RUN_ID, "--network-alias", "gluetun", "--cap-add", "NET_ADMIN",
                "--device", "/dev/net/tun:/dev/net/tun",
                "--env", "VPN_SERVICE_PROVIDER=custom", "--env", "VPN_TYPE=wireguard",
                "--env", f"VPN_PORT_FORWARDING={forwarding}", "--env", "VERSION_INFORMATION=off",
                "--env", "VPN_PORT_FORWARDING_PROVIDER=private internet access",
                "--env", "VPN_PORT_FORWARDING_USERNAME", "--env", "VPN_PORT_FORWARDING_PASSWORD",
                "--env", "PRIVATEERR_AUTO_RECOVER=true", "--env", "PRIVATEERR_GLUETUN_API_KEY",
                "--volume", f"{config}:/gluetun", "--volume", f"{wrapper}:/wrapper.sh:ro",
                "--entrypoint", "/bin/sh", GLUETUN, "/wrapper.sh",
            ).stdout.strip()
            containers.append(gluetun)
            docker("start", gluetun)
            probe = docker(
                "create", "--name", f"{RUN_ID}-probe", "--label", f"{LABEL}={RUN_ID}",
                "--network", RUN_ID, "--volume", f"{directory}:/test:ro",
                "--entrypoint", "sleep", IMAGE, "1800",
            ).stdout.strip()
            ...
            def request(method, route, body=None, authenticate=True):
                ...
                args = ["exec", "-i", probe, "curl", "--silent", "--connect-timeout", "2",
                        "--max-time", "20", "--write-out", "\n%{http_code}", "--request", method]
                if authenticate:
                    args.extend(["--config", "/test/curl-auth"])
                if body is not None:
                    args.extend(["--header", "Content-Type: application/json", "--data-binary", "@-"])
                args.append(f"http://gluetun:8000{route}")
                response = docker(*args, stdin=json.dumps(body) if body is not None else None, check=False)
            ...
            update = {
                ...
                "provider": {"server_selection": {
                    "names": ["example-two"], "wireguard": {
                        "endpoint_ip": "192.0.2.2", "endpoint_port": 51821,
            ...
            assert active["provider"]["server_selection"] == {
                **active["provider"]["server_selection"], **update["provider"]["server_selection"],
            }
            ...
            assert request("PUT", "/v1/vpn/settings", {"wireguard": {"private_key": "invalid"}})[0] == "400"  # pragma: allowlist secret
            assert json.loads(request("GET", "/v1/vpn/settings")[1])["wireguard"]["private_key"] == new_key
            print("PASS: real Gluetun authenticated API updates all connection fields without restarting its container.")
            print("PASS: unrelated settings preserved; invalid updates rejected; network namespace unchanged.")
            print("PIA connectivity and port forwarding are not exercised by this credential-free test.")
        finally:
            ...
            if network:
                ...
                if result.returncode == 0 and json.loads(result.stdout)[0]["Labels"].get(LABEL) == RUN_ID:
...
def live_recovery(gluetun, probe, privateerr, containers, config, request):
    ...
    def healthy():
        result = docker("exec", probe, "curl", "-fsS", "--max-time", "5",
                        "http://gluetun:9999", check=False)
    ...
    dependent = docker("create", "--name", f"{RUN_ID}-dependent", "--label", f"{LABEL}={RUN_ID}",
                       "--network", f"container:{gluetun}", "--entrypoint", "sleep", IMAGE, "1800").stdout.strip()
    ...
    internet = docker("exec", dependent, "curl", "-fsS", "--max-time", "20", "https://api.ipify.org")
    ...
    print("PASS: initial PIA tunnel, forwarding, and shared-network client connectivity.", flush=True)
    ...
    leak = docker("exec", dependent, "curl", "-kfsS", "--interface", "eth0", "--max-time", "5",
    ...
    print("PASS: blocked endpoint caused an outage; direct non-VPN traffic remained blocked.", flush=True)
    for _ in range(150):
    ...
    print("PASS: new PIA endpoint and keys, matching saved metadata, restored forwarding and dependent connectivity.", flush=True)
    print("PASS: Gluetun and the dependent container retained their identities and start times.", flush=True)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test isolated Gluetun recovery and clean up labeled resources.")
    parser.add_argument("--env-file", type=Path, help="Enable live PIA tests using credentials from this Compose environment file")
